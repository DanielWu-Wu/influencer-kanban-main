...
- **Gmail 与腾讯企业邮箱**
  ...
  - AI 同时生成外文与中文对照后可直接保存到当前邮箱草稿；中文或目标语言变化时必须重新翻译，外文可继续手动加入表情、调整措辞或排版并直接保存草稿。
...
- **每日待办与工作日历**
  ...
  - 腾讯只依据标准回复关系头判断是否已回复；相同主题的无关邮件不会误判。正文摘要不下载附件，AI 摘要失败不影响任务展示。
  - 任务键、完成状态和摘要缓存按 provider、邮箱账号和真实邮件定位隔离；新外部来信晚于完成时间时会重新进入待完成。
  - 进入每日待办时先显示已保存结果并在需要时后台静默刷新；任务行只显示邮箱类型，顶部读取状态保留具体账号。右箭头只切换到原账号并打开原线程/文件夹/UID，不修改飞书数据。
  - Gmail 和腾讯邮件正文成功显示后会标记当前真实会话为已读；选中邮件为白色背景并保留左侧蓝线。
