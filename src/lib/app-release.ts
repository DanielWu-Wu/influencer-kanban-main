...
export const APP_RELEASES: readonly AppRelease[] = [
  {
    ...
    title: '任务协同与邮件体验升级',
    highlights: [
      '项目提醒接入每日待办与工作日历，可统一查看、完成或恢复，并返回对应的合作项目或已确认绑定的邮件。',
      '手动任务支持静默云端同步；网络异常时保留本机待同步内容，恢复联网、页面可见或重新加载后继续补传。',
      '优化邮件回复体验：新增邮件草稿标识、附件展示和时间选择；调整 AI 回复策略时保留原正文、回复想法和保存入口。',
      '邮箱刷新更加安静稳定：返回邮箱页面时优先保留已有内容并在后台更新，减少重复读取和画面清空。',
      '飞书建档减少重复确认：信息明确且查重通过时直接后台处理，发现重复、冲突或覆盖风险时仍由人工确认。',
      '统一待办与邮件统计，减少不同页面之间的状态差异。邮件发送、邮箱草稿及需要确认的飞书操作仍由用户最终确认。',
    ],
  },
  {
    version: '1.2.7',
