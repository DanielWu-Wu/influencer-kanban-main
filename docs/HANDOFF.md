...
> 更新时间：2026-08-12
>
> 当前已提交基线：`main` / `5bcd6f0`（Gmail、Follow Up 与每日待办稳定性和交互优化），与 `origin/main` 一致。
> 本次洁癖收尾前主工作区干净；收尾只修改 `README.md` 与本文件，不提交、推送或部署。
>
...
- AI 同时生成的外文和中文对照未被修改时，两者视为已同步，可直接保存 Gmail 草稿；修改中文、外文或目标语言后，必须点击“根据中文更新外文”重新同步才能保存。
- “修改中文”只打开中文编辑；直接发送继续要求用户先完成中文人工确认和外文同步，允许直接保存高质量草稿不会扩大直接发送权限。
...
| 事实面 | 状态 | 当前证据 |
| --- | --- | --- |
| 代码基线 | verified-current | 2026-08-12：`main` / `5bcd6f0`，与 `origin/main` 对齐；本次知识收尾前工作区干净 |
| TypeScript | changed-and-verified | 2026-08-12：bundled Node 运行 `tsc -p tsconfig.json` 通过 |
| ESLint | changed-and-verified | 2026-08-12：本轮涉及的 Follow Up、每日待办、Gmail 双语草稿相关文件均通过 ESLint |
| 纯逻辑测试 | changed-and-verified | 2026-08-12：现有测试 128/128 通过，覆盖回复锚点、双语草稿同步、Follow Up 和 Gmail 待办状态 |
| 生产构建 | changed-and-verified | 2026-08-12：完整 Next.js 生产构建通过，包含 TypeScript、页面数据收集及 36 个页面生成 |
...
| 部署 | pending | `5bcd6f0` 已在 `origin/main`，但本轮未取得 Vercel deployment marker，也未验证 canonical 生产页面，不能据此推导已部署或 live verified |
...
5. Gmail AI 回复验证画像分析不会自动覆盖草稿、当前版/优化版可人工选择；未修改的中外文版本可直接保存草稿，任一内容或目标语言变化后必须重新同步；直接发送仍要求人工确认。
...
- 主工作区为 `main...origin/main`、基线 `5bcd6f0`；本次收尾前工作区干净，收尾后只有 `README.md` 与 `docs/HANDOFF.md` 的知识同步，不会提交、推送或部署。
...
当前交接基线是 main / 5bcd6f0，与 origin/main 对齐。本次洁癖收尾只同步
...
2026-08-12 当前 TypeScript、相关文件 ESLint、128/128 纯逻辑测试及完整 Next.js 生产构建均通过。
5bcd6f0 已在 origin/main，但尚未核对 Vercel marker，也未完成登录后的 Gmail/飞书/AI/每日待办真实账号验收。

下一阶段优先事项：
- 先用真实团队成员账号验收每人独立飞书企业应用授权、三表映射和账号隔离；
- 验收 Follow Up 审核/保存/飞书幂等写回、Gmail 老邮件搜索和任意邮件回复；
- 验收宽屏 Follow Up 批量审核、双语草稿直接保存/修改后重同步、线程切换无空白，以及每日待办右侧完成/恢复按钮；
