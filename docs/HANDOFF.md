...
> 更新时间：2026-08-14
>
> 当前已提交基线：`main` / `8d1c125`，与 `origin/main` 一致。`43e4194` 已包含 Gmail 双语草稿精简、
> Mailsuite 已发送邮件误追踪保护和 AI 模板事实输入优化；后续提交已完成“AI 系统功能提示词”改名、
> 移除旧“添加红人”入口，以及 AI 回复模板窗口在浏览器 100% 缩放下的滚动修复。
...
| 事实面 | 状态 | 当前证据 |
| --- | --- | --- |
| 代码基线 | verified-current | 2026-08-14：`main` / `8d1c125` 与 `origin/main` 对齐；近期功能代码均已提交，洁癖收尾前无未提交源码差异 |
| TypeScript | changed-and-verified | 2026-08-14：bundled Node 运行 `tsc -p tsconfig.json --noEmit` 通过 |
| ESLint | changed-and-verified | 2026-08-14：近期涉及的 `page.tsx`、`ai-reply-template-manager.tsx`、模板起草组件及两条 AI 路由定向 ESLint 通过 |
| 纯逻辑测试 | changed-and-verified | 2026-08-14：现有测试 134/134 通过 |
| 生产构建 | pending | `43e4194` 阶段曾完成完整构建；最新 `8d1c125` 的直接 Next.js 构建已完成编译，随后被本机 `spawn EPERM` 阻断，尚不能把最新 HEAD 标记为完整生产构建通过 |
| Supabase 账号迁移 | verified-current | 2026-08-07：用户在生产 SQL Editor 执行迁移，校验查询显示账号表、用户数据表、RLS、策略和检查函数均存在 |
| 浏览器交互 | pending | 2026-08-14：已在旧线上版本复现 AI 模板弹窗 100% 缩放时底部不可达，并完成本地滚动容器修复；修复后的登录态页面、100% 缩放和不同窗口高度仍待部署后人工验收 |
...
| 部署 | pending | `8d1c125` 已在 `origin/main`，但本轮未核对 Vercel 构建记录或线上版本标记；不能仅凭已推送断言生产站已部署最新改动 |
...
1. 部署后以浏览器 100% 缩放打开“管理 AI 回复模板”，编辑一个长模板，确认内容区可用鼠标滚轮滚到“默认语气”，标题和底部按钮保持可见，并在 90%/100% 及不同窗口高度下复测。
2. 确认顶部不再出现“添加红人”，红人列表不再显示误导性本地数量角标；在飞书多维表格新增一条测试红人后刷新列表，确认单一数据源流程正确。
3. 用两个测试邮箱人工验收 Mailsuite：从看板保存草稿并在 Gmail 网页发送，收件箱暂不打开；先在看板查看 `SENT` 邮件，确认没有新增打开记录且内嵌产品主图正常；再由收件邮箱真实打开并确认追踪正常。最后单独测试人工“显示远程图片”可能触发打开记录。
4. 用一个真实成员账号完整验证：保存自己的飞书 App ID/App Secret → 一键授权 → 配置三张表 → 只读检查/自动映射 → 人工确认一次写回 → 切换管理员确认数据、授权和密钥不串号。
5. Follow Up 单条和批量分别验证：后台生成不建草稿、中文修改后必须翻译、保存前二次检查、Gmail 草稿成功后写回飞书、写回失败重试不重复建草稿。
6. Gmail 验证较早邮件全局搜索、选择较早来信回复、自己发出的多收件人邮件、无飞书匹配邮件，以及草稿是否进入正确线程。
7. Gmail AI 回复验证画像分析不会自动覆盖草稿、当前版/优化版可人工选择；模板模式生成外文与中文后可直接保存，修改中文或目标语言后必须更新外文，外文人工调整后仍可保存且覆盖翻译前有提示。
8. 真实账号验收必须由用户在页面明确点击；不得用自动化测试创建 Gmail 草稿或写飞书。

## 6. Git、工作区与清场预览

- 主工作区为 `main...origin/main`、已提交基线 `8d1c125`。本次洁癖收尾前没有未提交源码差异；仅本文件会产生文档差异，不提交、推送或部署。
...
当前已提交基线是 main / 8d1c125，与 origin/main 对齐。
近期提交已完成：Gmail/AI 模板起草精简、Mailsuite 已发送邮件远程资源保护、AI 系统功能提示词改名、移除旧“添加红人”入口，以及 AI 回复模板弹窗在浏览器 100% 缩放下的滚动修复。
本次洁癖收尾只同步 docs/HANDOFF.md，没有提交、推送或部署。
...
2026-08-14 当前 TypeScript、相关文件 ESLint 和 134/134 纯逻辑测试通过。
看板查看每封带 SENT 标签的已发送邮件时默认阻止远程图片/背景/样式资源；内嵌产品主图保留，人工显示远程图片只对当前邮件临时生效且可能触发追踪。
AI 模板事实短内容可在紧凑框中直接输入，长内容由独立按钮打开大编辑器，两处实时共享内容且关闭不丢当前会话输入。
最新代码已推送到 origin/main，但是否已由 Vercel 部署、以及登录后的真实 Gmail/Mailsuite、AI 模板双语流程、100% 缩放滚动交互仍未验收；不要把“已推送”等同于“线上已生效”。

下一阶段优先事项：
- 先确认 Vercel 是否部署 8d1c125，并在 100% 缩放下验收 AI 模板窗口可以滚动到默认语气和保存按钮；
- 确认顶部旧“添加红人”入口和红人列表数量角标消失，新增红人统一在飞书多维表格完成；
- 验收 AI 模板双模式输入：短内容原位输入不弹窗，大编辑器继承内容并聚焦末尾，完成、右上角关闭和 Esc 均保留输入；
- 先用两个测试邮箱真实验收：看板默认打开 SENT 邮件不新增 Mailsuite 打开记录、内嵌产品图正常、收件人真实打开仍能被追踪；
- 先用真实团队成员账号验收每人独立飞书企业应用授权、三表映射和账号隔离；
- 验收 Follow Up 审核/保存/飞书幂等写回、Gmail 老邮件搜索和任意邮件回复；
- 验收宽屏 Follow Up 批量审核、双语草稿外文人工润色后直接保存、线程切换无空白，以及每日待办右侧完成/恢复按钮；
