...
> 更新时间：2026-08-13
>
> 当前已提交基线：`main` / `9bb01cc`，与 `origin/main` 一致；该提交已包含 Gmail 双语草稿修复。
> 基线上另有本次已验证但未提交的 Mailsuite 已发送邮件误追踪保护，以及 AI 模板起草双模式事实输入；洁癖收尾同步 `README.md` 与本文件，不提交、推送或部署。
>
...
| 事实面 | 状态 | 当前证据 |
| --- | --- | --- |
| 代码基线 | changed-and-verified | 2026-08-13：`main` / `9bb01cc` 与 `origin/main` 对齐；另有未提交代码实现 Mailsuite 已发送邮件远程资源保护、AI 模板起草双模式输入、测试和文档同步 |
| TypeScript | changed-and-verified | 2026-08-13：bundled Node 运行 `tsc -p tsconfig.json` 通过 |
| ESLint | changed-and-verified | 2026-08-13：全项目 ESLint 通过，0 error；仍保留 7 个既有 warning |
| 纯逻辑测试 | changed-and-verified | 2026-08-13：现有测试 133/133 通过；新增覆盖远程 URL、`srcset`、CSS 资源、内嵌图片保留和仅 `SENT` 邮件启用保护 |
| 生产构建 | changed-and-verified | 2026-08-13：完整 Next.js 生产构建通过，包含 TypeScript、页面数据收集及 36 个页面生成 |
| Supabase 账号迁移 | verified-current | 2026-08-07：用户在生产 SQL Editor 执行迁移，校验查询显示账号表、用户数据表、RLS、策略和检查函数均存在 |
| 浏览器交互 | pending | 2026-08-13：独立 `localhost:5001` 开发服务可返回页面且控制台无错误，但 in-app Browser 停在账号加载界面，Chrome 扩展会话不可用；没有进入登录后的 Gmail 邮件详情，不能据此声称 Mailsuite 或 AI 模板双模式交互验收通过 |
...
| 部署 | pending | 已提交基线 `9bb01cc` 在 `origin/main`；本次 Mailsuite 误追踪保护尚未提交、推送或部署，也未核对 Vercel marker 和 canonical 生产页面 |
...
1. 用两个测试邮箱人工验收 Mailsuite：从看板保存草稿并在 Gmail 网页发送，收件箱暂不打开；先在看板查看 `SENT` 邮件，确认没有新增打开记录且内嵌产品主图正常；再由收件邮箱真实打开并确认追踪正常。最后单独测试人工“显示远程图片”可能触发打开记录。
2. 用一个真实成员账号完整验证：保存自己的飞书 App ID/App Secret → 一键授权 → 配置三张表 → 只读检查/自动映射 → 人工确认一次写回 → 切换管理员确认数据、授权和密钥不串号。
3. Follow Up 单条和批量分别验证：后台生成不建草稿、中文修改后必须翻译、保存前二次检查、Gmail 草稿成功后写回飞书、写回失败重试不重复建草稿。
4. Gmail 验证较早邮件全局搜索、选择较早来信回复、自己发出的多收件人邮件、无飞书匹配邮件，以及草稿是否进入正确线程。
5. 合作项目分别验证：线程最后是普通我方回复、Gmail 表情回应、已有草稿三种情况；连续保存两次应只有一份草稿且第二次更新内容，物流日期应保持飞书记录的上海业务日期。
6. Gmail AI 回复验证画像分析不会自动覆盖草稿、当前版/优化版可人工选择；AI 模板事实短内容可原位输入，独立按钮打开大编辑器后内容和光标位置正确，三种关闭方式均不丢内容；修改中文并更新外文后，再在外文加入表情或润色应仍可保存草稿；再次修改中文或目标语言才要求重新翻译，直接发送仍要求人工确认。
7. Gmail 签名三种范围分别检查开发信、Follow Up、正常新邮件/回复和合作项目邮件，确认 AI 不再重复姓名/落款。
8. 每日待办验证右侧勾勾把 Gmail 来信移入“今日已完成”、右侧转圈恢复到待完成；同时确认自动回复完成、新来信重新待办、三组折叠、中文摘要和箭头跳转保持正确。
9. 工作日历验证点击日期只查看详情，“新增日程”独立工作，合作节点头像、日期圆心和项目跳转正确。
10. 真实账号验收必须由用户在页面明确点击；不得用自动化测试创建 Gmail 草稿或写飞书。

## 6. Git、工作区与清场预览

- 主工作区为 `main...origin/main`、已提交基线 `9bb01cc`；本次功能修改 `src/components/email-detail.tsx`、`src/lib/email-remote-content.ts`、`tests/email-remote-content.test.ts`、`tests/run-tests.ts` 与 `src/components/ai-template-reply-composer.tsx`，洁癖收尾另同步 `README.md` 与本文件，均未提交、推送或部署。
...
当前已提交基线是 main / 9bb01cc，与 origin/main 对齐，已包含 Gmail 双语草稿修复。
本次另有未提交代码完成 Mailsuite 已发送邮件误追踪保护和 AI 模板起草双模式事实输入；
洁癖收尾同步 README.md 和 docs/HANDOFF.md，没有提交、推送或部署。
...
2026-08-13 当前 TypeScript、全项目 ESLint（0 error、7 个既有 warning）、133/133 纯逻辑测试及完整 Next.js 生产构建均通过。
...
本次修复尚未提交或部署；本地浏览器停在账号加载界面且 Chrome 扩展会话不可用，尚未完成登录后的真实 Gmail/Mailsuite 和双模式输入验收。

下一阶段优先事项：
- 先验收 AI 模板双模式输入：短内容原位输入不弹窗，大编辑器继承内容并聚焦末尾，完成、右上角关闭和 Esc 均保留输入；
