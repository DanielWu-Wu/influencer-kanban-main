...
> 历史交接说明：当前最新交接入口是根目录 `HANDOFF.md`。本文件保留较多历史上下文，后续开发应优先以 `HANDOFF.md` 和实际代码为准。
...
3. **开发信**
   ...
   - 可单独重新生成标题或正文。
   - 可预览产品链接、产品主图、签名链接。
   - 当前代码保存 Gmail 草稿后，线索自动移出开发信队列，并自动把飞书双表“初次开发信”标记为“已发”。
   - 这里存在业务语义风险：Gmail 草稿不等于真实发送，后续应改成用户确认实际发送后再写回“已发”。
   - 邮件仍需用户进入 Gmail 手动检查和发送。

4. **开发信跟进**
   - 查看最近 7、10、14、30 天的飞书开发记录。
   - 第 3 天进行二次跟进，第 7 天进行三次跟进；没有第 14 天跟进。
   - 检查 Gmail 回复，区分人工回复、自动回复、退信和未找到初次开发信。
   - 跟进草稿只保存到 Gmail，用户真实发送后再手动确认写回飞书。

## 当前已完成的功能

### 红人开发台

- 从杂糅工作台重构为 4 个流程 Tab：红人录入、邀约确认、开发信、开发信跟进。
- 支持两套飞书表配置：
  - 红人资源库：连接“红人信息数据库”。
  - 红人开发记录表：连接“红人开发情况表”。
- 新增 Supabase 本地表：
...
- 保存 Gmail 草稿后：
  - 线索移出开发信队列。
  - 当前代码会自动写回飞书双表“初次开发信 = 已发”。
  - Gmail 不自动发送。
  - 后续应统一“真实已发送”确认机制，避免把草稿误计为已发送。
...
- 首封邮件“保存 Gmail 草稿”与“真实已发送”语义不一致。二次/三次跟进已经采用真实发送后手动写回，首封邮件也应统一。
...
3. 统一首封邮件真实发送确认机制，避免保存 Gmail 草稿后直接被统计为已发。
4. 实测保存 Gmail 草稿后只创建草稿、不自动发送，并确认当前飞书“已发”写回行为是否需要保留到过渡期。
5. 如果第 1-4 步稳定，再给“开发记录疑似重复”增加同类 Hover 预览。
6. 优化 Gmail 草稿创建失败提示，尽量把 Gmail API 返回的真实原因展示出来。
7. 整理设置页字段映射状态，让用户更清楚哪些字段缺失会影响哪些流程。
